...
def tabu_search(): #全局变量用于传递初始路径和计算路径分值
    ...
    for iter in range(0,50): #增加迭代次数
        bestscore = float('inf') #保存当前邻域中找到的最优解的分值
        best_candidate = None
        v1, v2 = -1, -1
        ...
        for i in range(1, n-1):
            for k in range(i+1, n):
                temp_route = two_opt_swap(way, i, k)
                bestway = temp_route
                score = comp_bestway_score()
                
                if score < obscore:
                    obscore = score
                    obway = temp_route.copy()
                    way = temp_route.copy()
                    improved = True
                    break
            if improved:
            ...
            for a in range(1, n):
                for b in range(a):
                    ...
                    if(score < obscore):
                        ...
                        bestscore = score
                        best_candidate = bestway.copy()
                        v1, v2 = a, b
                if improved:
                    break
            
            # 如果没有找到更优解，但找到了可接受的候选解
            if not improved and best_candidate is not None and v1 != -1:
                way = best_candidate.copy()
