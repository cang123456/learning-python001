...
def sa():
    t = 1000    # 提高初始温度，增加搜索范围
    tf = 0.001  # 降低终止温度，提高精度
    kb = 0.95   # 减小降温系数，更缓慢降温
    ...
    while t > tf:
        for iter in range(500):  # 增加等温过程的迭代次数
            # 随机选择两个不同的城市进行交换
            ...
            if(v1 != v2):
                ...
                # 额外的2-opt优化尝试
                if random.random() < 0.3:  # 30%概率尝试2-opt优化
                    i = random.randint(1, n-2)
                    k = random.randint(i+1, n-1)
                    temp_route = two_opt_swap(bestway, i, k)
                    
                    # 计算新路径的分数
                    temp_score = 0.0
                    for idx in range(n):
                        temp_score += dis_array[temp_route[idx]][temp_route[(idx+1)%n]]
